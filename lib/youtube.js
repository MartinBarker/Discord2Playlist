...
async function createPlaylist(youtube, scanJobId, create) {
  ...
  const title = opts.title || `Discord2Playlist — scan ${scanJobId}`;
  const snippet = {
    title,
    description:
      opts.description ||
      'Created by discord2playlist (https://martinbarker.me/discord2playlist).',
  };
...
async function insertWithRetry(youtube, playlistId, videoId, onRetry = () => {}, attempt = 0) {
  ...
  try {
    ...
    if (info.transient && attempt < MAX_ATTEMPTS) {
      ...
      return insertWithRetry(youtube, playlistId, videoId, onRetry, attempt + 1);
    }
...
//
// Events: start, progress, retry, aborted, done, error.
// Returns { playlistId, inserted, failed, skipped, aborted }.
async function pushPendingToPlaylist(scanJobId, discordUserId, emit = () => {}, options = {}) {
  ...
  for (const { media_id } of todo) {
    try {
      const result = await insertWithRetry(youtube, target.id, media_id, retry =>
        emit('retry', { mediaId: media_id, ...retry })
      );
  ...
  return { playlistId: target.id, inserted, failed, skipped, aborted: null };
}
